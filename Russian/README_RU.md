...
Также, имейте в виде, что многие вопросы не требуют однозначного короткого ответа, а лишь помогают завести беседу на ту или иную тему (предоставляя кандидату возможность показать себя во всей красе).
...
* В чем разница между "атрибутом (attribute)" and a "свойством (property)"?
...
* В чем разница между `.bind()`, `.live()`, and `.delegate()`?
* В чем разница между `$` and `$.fn`? Что вообще такое `$.fn`?
