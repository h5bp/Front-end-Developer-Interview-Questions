...
* Кого із світових front-end розробників Ви знаєте?
...
* Розкажіть про свою середу розробки (ОС, редактор, браузер(и), інші інструменти)
...
* Як можна оптимізувати загрузку зовнішніх ресурсів на сторінці?
	* Комбінація із потенційних рішень:
		...
		* Кешування
		* і т.д.
* Яка перевага в підкачці із зовнішніх ресурсів з декількох доменів?
...
* Якщо Ви прийшли на проект, де для форматуровання використовується табуляція, а Ви звикли використовувати пробіли, як Ви поступете?
...
* Чи знакомі ви з шаблонізаторами (Smarty, HAML, т.д.)
...
* Де досі використовується `document.write ()`?
	* У більшості генеруємих банерів, хоча так робити не рекомендується
...
* Назвіть 4 різних види аргументів, які приймає функція jQuery()?
	* Селектор (строка), HTML (строка), Callback (функція), HTMLElement, об'єкт, масив, масив елементів, об'єкт jQuery etc.
...
* Які ви знаєте методи заборони обтікання (clearing) і які де застосовуються?
...
* Ви коли-небудь використовували сітковий верстку (grid system, grid design)? Якщо так, яка Ваша улюблена?
...
* Як Ви зверстаєте макет, який використовує нестандартні шрифти?
	* Webfonts (сервиси на кшталт Google Webfonts, Typekit, Fontsquirrel etc.)
...
* Що Вам найбільше подобаються в Ваших інструментах розробника?
* У Вас є якісь особисті проекти?
* Візьміть аркуш паперу і напишіть в стовпчик букви A B C D E. Тепер відсортируйте стовпчик в алфавітному порядку за спаданням, не написавши жодного рядка коду.
	* Засічіть, через скільки часу кандидат переверне аркуш
* Пірат або ніндзя?
	* Бонус за комбінацію. Аргументовану. +2 за зомбі-пірат-ніндзя-мавпу
* Чим би Ви займалися, якби не Web-розробкою?
